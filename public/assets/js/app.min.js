...
function validatorHandler(ResponseError, element){
    error = JSON.stringify(ResponseError.responseJSON).split("Error:").map(function(element, index) {
        if (index == 0)
            return null;
    ...
    error += `<li>${ResponseError.responseJSON.messages ?? ""}</li>`
